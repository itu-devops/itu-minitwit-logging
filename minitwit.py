...
def before_request():
    ...
    """
    logging.debug(f"Serving request to: {str(request.remote_addr)}")
...
def public_timeline():
    ...
    # The following code randomly generates a break, so that we can see a
    # corresponding error message every now and then
    if randint(0, 3) == 3:
...
def user_timeline(username):
    ...
    if profile_user is None:
        msg = (
            f"Unknown user {username} tried to connect from "
            + f"{str(request.remote_addr)}"
        )
        logging.debug(msg)
        abort(404)
    followed = False
